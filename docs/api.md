...
- `USER_PARTICIPATED`：リクエストボディのユーザー発言を保存した上で、まずそのユーザー発言自体を`message`イベントとして配信し、続けて文脈に応じてLLMが選択したペルソナの応答をSSE（Server-Sent Events）でストリーミング配信する。応答後、LLMが「会話を続けるべき」と判断した場合は、別のペルソナが連鎖して発言することがある（安全弁として連鎖回数の上限を設ける）。「連鎖が自然に終わる」か「上限に達する」か「ユーザが会話を停止させる」と、制御はユーザーに戻る。ユーザー発言の本文は最大500文字とし、空文字は不可（400を返す）。
...
| `message` | ペルソナ・ユーザーいずれかの発言が1件完成し、保存済みであることを示す（`message`：`GET /chats/{chat_id}/messages`と同じ形式）。`USER_PARTICIPATED`では、ストリーム開始直後にリクエストしたユーザー自身の発言が最初の`message`イベントとして届く。 |
| `title` | チャットタイトルが自動生成・更新されたことを示す（`title`）。ペルソナの応答を待たず、`USER_PARTICIPATED`ではユーザーの最初の発言、`PERSONA_ONLY`では事前入力されたお題のみを根拠に、会話の一番序盤（最初の`thinking`より前）で最大1回のみ配信される。 |
